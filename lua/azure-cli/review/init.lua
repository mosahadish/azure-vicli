...
local function setup_overview_keymaps(buf)
  ...
  KEYS.bind(buf, "overview", "search", function() EXT.nav.search_files() end, { desc = "search text across the PR's changed files" })
...
local function setup_diff_keymaps(buf)
  ...
  KEYS.bind(buf, "diff", "goto_definition", function() EXT.nav.goto_definition() end, { desc = "go to definition" })
  KEYS.bind(buf, "diff", "find_references", function() EXT.nav.find_references() end, { desc = "find references" })
  KEYS.bind(buf, "diff", "open_file", function() EXT.nav.open_file() end, { desc = "open the current file at the PR's revision" })
  KEYS.bind(buf, "diff", "search", function() EXT.nav.search_files() end, { desc = "search text across the PR's changed files" })
...
-- Code navigation (gd/gr/gf/g-slash, the peek view and the revision
-- buffers it opens) lives in lua/azure-cli/review/nav.lua - loaded by the
-- closing block at the end of this file, like every other review/*
-- module, and reached from here through the four nav_* forward
-- declarations above plus EXT.nav. It was ~640 lines and 30 of this
-- file's top-level locals; see that module's own header comment.
-- ---------------------------------------------------------------------------
...
toggle_ignore_ws = function()
  ...
  for _, buf in pairs(EXT.nav.bufs) do
    if vim.api.nvim_buf_is_valid(buf) then EXT.nav.decorate_revision(buf) end
  end
...
KEYS.bind(list_buf, "list", "search", function() EXT.nav.search_files() end, { desc = "search text across the PR's changed files" })
...
;(function()
  ...
  -- ctx.show_hits / open_revision / ensure_revision_buf / when_loaded /
  -- nav_context / nav_show / nav_back are review/nav.lua's, filled in just
  -- after it loads (see EXT.nav below) rather than here - the modules that
  -- read them all load after it.
  ...
  -- Everything review/nav.lua reads out of this file. All read-only there,
  ...
  -- Code navigation, loaded before every other module: they reach the peek
  -- view and the revision buffers through the ctx fields filled in right
  -- below, and this file's own gd/gr/gf/g-slash bindings (forward-declared
  -- at the top) are this module's too.
  EXT.nav = require("azure-cli.review.nav")(ctx)
  ctx.show_hits = EXT.nav.show_hits
  ctx.open_revision = EXT.nav.open_revision
  ctx.ensure_revision_buf = EXT.nav.ensure_revision_buf
  ctx.when_loaded = EXT.nav.when_loaded
  ctx.nav_context = EXT.nav.context
  ctx.nav_show = EXT.nav.show
  ctx.nav_back = EXT.nav.back
