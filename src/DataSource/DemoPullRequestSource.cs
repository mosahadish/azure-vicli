...
#pragma warning restore CA5394 // Do not use insecure randomness
        ...
        public async IAsyncEnumerable<PullRequestViewElement> FetchGroupedPullRequests()
        {
            ...
            yield return Fake("DataSouce: Skip reviews that the reviewer has declined", state: PrState.SignedOff);
            yield return Fake("Docs: Try to syntax highlight config in README.md", state: PrState.SignedOff);
            yield return Fake("Docs: Update README.md", state: PrState.Waiting);
            yield return Fake("Feature: Add 's' hot key to filter view to signedoff pull requests", state: PrState.SignedOff);
            yield return Fake("Switch build version", state: PrState.Drafts);
            yield return Fake("Docs: Document AAD feature in README.md", state: PrState.Waiting);
        ...
        {
            await Task.CompletedTask;
            return $"Re-queued build validation for PR {pullRequestId} (demo).";
        }
        ...
        {
            await Task.CompletedTask;
            return new UserIdentity(Guid.Empty, "Brian Gianforcaro");
        }

        private PullRequestViewElement Fake(string title, string name = "Brian Gianforcaro", PrState state = PrState.Actionable)
        {
            ...
            return new PullRequestViewElement(pr) { State = state };
        }
